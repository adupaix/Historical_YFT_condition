...
coef.df <- list() ; names(coef.df) <- vars_of_model
p.val.df <- list() ; names(p.val.df) <- vars_of_model
...
#' hence the gam was performed on a subset of the data_filtered dataset. The levels of the explanatory variables
#' can then differ. So we take the ones from the GAM dataset (saved in gam$model)
if (append_readme){lev <- levels(data$fishing_year)}
...
if (append_readme){lev <- levels(data$fishing_quarter)}
